...
> 面向中小企业的开源私有化知识库：单实例部署、数据本地可控、LLM 中立。LangChain4j RAG、混合检索、MySQL + 本地向量、Linux 一键部署。

## 推荐 Topics

`knowledge-base` · `rag` · `langchain4j` · `mysql` · `self-hosted` · `private-deployment` · `llm-agnostic` · `spring-boot` · `vue3` · `java21` · `open-source` 等（GitHub 上限 20 个）
