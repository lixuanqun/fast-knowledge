...
fast-knowledge/
├── LICENSE                     # GNU AGPL v3
├── LICENSE-COMMERCIAL.md       # 商业双许可说明
├── README.md
├── docs/
│   ├── 产品说明.md              # 产品定位与功能特性（本文档的详细版）
│   └── fast_knowledge.md       # 架构设计、数据模型、API
├── scripts/
│   ├── dev.ps1                 # Windows 开发
│   ├── build.ps1 / build-jar.ps1
│   └── install.sh              # Linux/macOS 一键 Docker 部署
├── deploy/
│   ├── docker-compose.yml      # MySQL + Redis
│   ├── docker-compose.full.yml # 全栈（应用 + 依赖，可选 Ollama / Qdrant）
│   ├── Dockerfile
│   └── .env.example
...
```powershell
cd deploy && docker compose up -d   # MySQL（Redis 可选）
cd ..
npm install
npm run dev
```
...
关键配置见 [deploy/.env.example](deploy/.env.example)：
