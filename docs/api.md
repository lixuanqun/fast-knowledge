...
> **产品背景**：[产品说明.md](./产品说明.md) — 中小企业私有化知识库，单实例部署；接口设计服务于知识库管理、混合检索与 RAG（检索/问答/对话/写文档），非多租户 SaaS API。
...
|------|------|
| 已登录 | 普通业务接口 |
| ADMIN | 用户管理、大模型配置相关接口（`@PreAuthorize("hasRole('ADMIN')")`） |
...
interface KnowledgeBase {
  ...
  searchAlpha: number       // 已废弃（保留兼容）。HYBRID RRF 检索不使用；请用 searchTopK
  searchTopK: number        // 默认 8，检索与 RAG 返回条数
  status?: number
  createdAt?: string
  updatedAt?: string
}
...
**响应 data**：`LlmConfig`（`apiKey` 仅以掩码形式返回）

---

#### PUT /system/llm-config

保存大模型配置并**立即生效**（热刷新 ChatModel，无需重启）。
...
#### POST /system/llm-config/test

使用请求体中的参数测试 LLM 连通性（可先测试未保存的表单值）。
...
| searchAlpha | number | 否 | **已废弃**（默认 0.6，不影响检索） |
...
| alpha | number | 否 | **已废弃**。保留兼容，当前 HYBRID RRF 不使用 |
...
| 12a | GET | /system/llm-config | ADMIN | 系统 | 大模型配置 |
| 12b | PUT | /system/llm-config | ADMIN | 系统 | 保存大模型配置 |
| 12c | POST | /system/llm-config/test | ADMIN | 系统 | 测试 LLM 连接 |
...
| v1.1 | 2026-07-03 | LangChain4j 重构：`searchAlpha`/`alpha` 标注废弃；补充 `rerankEnabled`/`rerankProvider`；检索说明改为 HYBRID+RRF |
