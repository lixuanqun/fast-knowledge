...
应用 + MySQL + Redis 一次启动，适合中小企业试用与私有化 POC。
...
空库首次启动会自动建表。若手动建过旧表，请对齐 [V1__baseline.sql](../../backend/src/main/resources/db/migration/V1__baseline.sql)。

### 问答无响应

检查 `LLM_PROVIDER` 与 API Key；Docker 全栈默认连 `ollama` 容器，需 `--profile ai` 并拉取模型。
