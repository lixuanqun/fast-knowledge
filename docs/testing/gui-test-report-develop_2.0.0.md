...
| ID | 级别 | 问题 | 处置 |
|----|------|------|------|
| BUG-1 | P0 | v2 默认维度 1024 与存量 pgvector 表 `vector(512)` 不匹配，索引以晦涩报错失败 | **已修复并提交**：启动时维度一致性校验（`PgVectorEmbeddingStoreConfig`），输出可操作指引；环境对齐后复测通过 |
| BUG-2 | P1 | LLM 缺位时问答/写文档失败反馈缺失或仅短暂 toast，结果区无持久错误 | 待修：建议失败信息持久展示在结果区；另 LLM 连接超时期间（最长 60s）无进度反馈 |
| BUG-3 | P2 | 错误密码登录无任何可见错误提示 | 待修：登录表单增加持久错误文案 |
| OBS-1 | P3 | 历史遗留数据存在 `?????` 乱码记录（仪表盘热门查询、知识库列表） | 旧数据编码问题，与本次改动无关，建议清理测试库 |
| OBS-2 | P3 | Element Plus 下拉选项 `?????`（同 OBS-1 数据） | 同上 |

**说明**：测试期间控制台错误日志未接入监听（工具无只读 console 能力），页面错误证据以可见表现 + 后端日志为准（后端日志捕获到 BUG-1 的完整 SQL 异常链）。
