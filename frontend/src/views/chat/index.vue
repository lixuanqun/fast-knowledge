    ...
    <div class="chat-layout">
      ...
      <main class="chat-main">
        ...
        <div ref="messagesRef" v-loading="messagesLoading" class="messages">
          <EmptyState v-if="!messages.length && !streaming && !messagesLoading" description="发送消息开始对话" :image-size="80" />
          ...
          <div v-if="streaming" class="msg-bubble assistant">
            ...
            <div class="msg-bubble__body">
              <MarkdownBody :content="streamText || '思考中...'" />
            </div>
          </div>
        </div>
...
async function removeSession(id: number) {
  await ElMessageBox.confirm('确认删除该会话？')
  try {
...
<style scoped>
.chat-page { display: flex; flex-direction: column; height: calc(100vh - 88px); }
.chat-layout { display: flex; gap: 16px; flex: 1; min-height: 0; }
.session-panel {
  width: 240px; background: #fff; border-radius: 10px; padding: 12px; overflow-y: auto; flex-shrink: 0;
}
...
.session-time { font-size: 11px; color: #aaa; margin-top: 4px; }
.chat-main { flex: 1; display: flex; flex-direction: column; min-width: 0; }
.chat-toolbar { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; }
.toolbar-label { font-size: 13px; color: #666; }
.messages {
  flex: 1; overflow-y: auto; background: #fff; padding: 16px; border-radius: 10px; margin-bottom: 12px;
}
...
.msg-bubble.user .msg-bubble__avatar { background: #409eff; }
.msg-bubble__body {
  max-width: 75%; background: #f6f7f8; padding: 12px 14px; border-radius: 10px;
}
.msg-bubble.user .msg-bubble__body { background: #ecf5ff; }
.msg-text { white-space: pre-wrap; line-height: 1.7; }
