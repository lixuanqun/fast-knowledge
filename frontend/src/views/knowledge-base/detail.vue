<template>
  <div class="page-container">
    <PageHeader
      :title="kb?.name || '知识库详情'"
      :subtitle="kb?.description || '管理文档、成员与检索配置'"
      show-back
    >
      <template #actions>
        ...
        >
          <el-button type="primary" :loading="uploadMutation.isPending.value">上传文档</el-button>
        </el-upload>
      </template>
    </PageHeader>
    ...
    <el-tabs v-else v-model="tab">
      <el-tab-pane label="文档" name="docs">
        <el-table v-if="docs.length" :data="docs" stripe>
          ...
          <el-table-column label="操作" width="220" fixed="right">
            <template #default="{ row }">
              <el-button link @click="openPreview(docRow(row))">预览</el-button>
              <el-button link @click="handleReindex(row.id)">重新索引</el-button>
              <el-button link type="danger" @click="handleDelete(row.id)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <EmptyState v-else description="暂无文档，点击右上角上传">
          <el-upload :show-file-list="false" :http-request="handleUpload" accept=".pdf,.docx,.txt,.md,.pptx,.xlsx,.html">
      ...
      <el-tab-pane label="成员" name="members">
        <el-form inline class="member-form">
          <el-input v-model="memberForm.username" placeholder="用户名" style="width:160px" />
        ...
        <EmptyState v-else description="暂无成员，可添加协作者" />
      </el-tab-pane>
      ...
      <el-tab-pane label="设置" name="settings">
        <el-form label-width="130px" style="max-width:560px">
          ...
          <el-form-item label="混合检索权重 α">
            <el-slider v-model="settingsForm.searchAlpha" :min="0" :max="1" :step="0.1" show-input />
            <div class="form-tip">越大越偏向向量语义，越小越偏向关键词</div>
          </el-form-item>
          <el-form-item label="检索 Top K">
            <el-input-number v-model="settingsForm.searchTopK" :min="1" :max="30" />
          </el-form-item>
      ...
      <el-tab-pane name="tasks">
        ...
        <el-table v-if="failedTasks.length" :data="failedTasks" stripe>
          ...
          <el-table-column label="操作" width="100">
            <template #default="{ row }">
              <el-button link @click="handleRetry(row.documentId)">重试</el-button>
            </template>
          </el-table-column>
        </el-table>
        <EmptyState v-else description="暂无失败任务" />
      </el-tab-pane>
    </el-tabs>
...
<style scoped>
.member-form { display: flex; gap: 8px; margin-bottom: 16px; flex-wrap: wrap; }
